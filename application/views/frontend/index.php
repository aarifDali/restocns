...
<!-- Best Burgers Section Start -->
<div class="best-burgers-section">
  <div class="container">
    <div class="section-title-wrap d-flex justify-content-between align-items-center">
      <div>
        <h2 class="title">Best Burgers</h2>
      </div>
      <div class="best-burgers-see-all">
    ...
    <div class="best-burgers-carousel-wrapper">
      <div class="best-burgers-carousel" id="best-burgers-carousel">
        <?php 
        // Fetch Burgers category items
        $categories = getFoodMenuCategory();
        $burgers_category = null;
        foreach($categories as $category) {
          if(strtolower($category->category_name) == 'burgers') {
            $burgers_category = $category;
            break;
          }
        }
        ...
        <?php 
            ...
            // Show message if no burgers available
        ?>
        <div class="no-burgers-message">
          <p>No burgers available at the moment.</p>
        </div>
        <?php 
          }
        } else {
          // Show message if no burgers category found
        ?>
        <div class="no-burgers-message">
          <p>Burgers category not found.</p>
        </div>
        <?php } ?>
      </div>
      
      <!-- Navigation Arrows Under Cards -->
      <div class="best-burgers-nav-bottom d-flex justify-content-end">
        <div class="best-burgers-nav">
          <a href="javascript:void(0)" class="customPrevBtn best-burgers-prev">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <a href="javascript:void(0)" class="customNextBtn best-burgers-next">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
document.addEventListener('DOMContentLoaded', function() {
  // Best Burgers Carousel
  const burgersCarousel = document.getElementById('best-burgers-carousel');
  const prevBtn = document.querySelector('.best-burgers-prev');
  const nextBtn = document.querySelector('.best-burgers-next');
  ...
  if (burgersCarousel && prevBtn && nextBtn) {
    ...
    burgersCarousel.addEventListener('touchend', function(e) {
      if (!isDragging) return;
      isDragging = false;
